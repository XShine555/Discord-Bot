...
class MusicPlayer(DiscCommands.Cog):
    ...
    async def getLocale(Self, Context, Locale):
        ...
        Identifier = Context.guild.id
        Language = Self.Cursor.execute('SELECT Language FROM General_Preferences WHERE Server_Identifier = ?', (Identifier,) ).fetchone()[0]

        File = open('MusicPlayer_Locale.json', encoding = 'UTF-8')
        Read = Json.load(File)

        return Read[Locale][Language]
    ...
    def getLanguages(Self):
        ...
        return Read['Available_Languages']
